...
public class ploblem_11 {
	final static int max_size = 19,min_size = 0;
	...
	public static void main(String[] args) {
		...
		for(int y = 0; y<max_size;y++)
		{
			for(int x = 0; x<max_size;x++)
			{
				if(check_right(x))
				{
					int  i = matrix[y][x] * matrix[y][x+1] * matrix[y][x+2]* matrix[y][x+3];
					checkMaxValue(i);
				}
				if(check_down(y))
				{
					int  i = matrix[y][x] * matrix[y+1][x] * matrix[y+2][x]* matrix[y+3][x];
					checkMaxValue(i);
				}
				if(check_down_left(y, x))
				{
					int  i = matrix[y][x] * matrix[y+1][x-1] * matrix[y+2][x-2]* matrix[y+3][x-3];
					checkMaxValue(i);
				}
				if(check_down_right(y, x))
				{
					int  i = matrix[y][x] * matrix[y+1][x+1] * matrix[y+2][x+2]* matrix[y+3][x+3];
					checkMaxValue(i);
				}
	...
	private static void checkMaxValue(int i) {
		// TODO Auto-generated method stub
		if(i>Max_value)
			Max_value = i;
	}

	static boolean check_right(int x) {
		if (x + 3 < max_size)
			return true;
		else
			return false;
	...
	static boolean check_down(int y) {
		if (y + 3 < max_size)
			return true;
		else
			return false;
	}
	static boolean check_down_left(int y,int x)
	{
		if (y + 3 < max_size && x - 3 > 0)
			return true;
		else
			return false;
	}
	static boolean check_down_right(int y,int x)
	{
		if (y + 4 < max_size && x + 4 <max_size)
			return true;
		else
			return false;
	}
